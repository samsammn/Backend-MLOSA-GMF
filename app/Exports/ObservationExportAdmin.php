...
class ObservationExportAdmin implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    /**
    ...
    */
    ...
    public function __construct($request)
    {
        if ($request->year !== null) {
            $this->filter[] = [DB::raw('year(o.observation_date)'), '=', $request->year];
        }

        if ($request->start_month != null){
            $this->filter[] = [DB::raw('month(o.observation_date)'), '>=', $request->start_month];
        }

        if ($request->end_month != null){
            $this->filter[] = [DB::raw('month(o.observation_date)'), '<=', $request->end_month];
        }

        if ($request->maintenance_process != null){
            $this->filter[] = [DB::raw('mp.id'), '=', $request->maintenance_process];
        }

        if ($request->uic != null){
            $this->filter[] = [DB::raw('o.uic_id'), '=', $request->uic];
        }
    }
